...
function ConfirmExtendVPS({ isOpen, onOpenChange }: any) {
  ...
  const tableVpsBuCloud: any = useAppSelector(
    (state) => state.table["vps_bu_cloud"]
  );
  const [list, setList] = useState<any[]>([]);

  // const list = useMemo(() => {
  //   console.log(tableVpsBuCloud?.selectedKeys);
  //   const listSelect = [...(tableVpsBuCloud?.selectedKeys || "")];
  //   return vpsList.filter((vps) => listSelect.includes(vps._id));
  // }, [tableVpstableVpsBuCloudVng]);

  const getInfoListVPSRenew = async () => {
    const result = await vpsApis.getInfoListVPSRenew({
      item: [...(tableVpsBuCloud?.selectedKeys || "")],
    });
    setList(result.data);
  };
  useEffect(() => {
  ...
  const handleCreateOrder = async () => {
    ...
    orderDomainLinkList.forEach((itemDomainList) => {
      list.forEach((listVPS, index) => {
        if (listVPS.team._id === itemDomainList._idTeam) {
          console.log(itemDomainList);
          list[index] = { ...listVPS, orderDomainLink: itemDomainList.value };
        }
      });
    });
    const newlist = list
