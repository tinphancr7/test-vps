...
function TableVpsVietStack() {
  ...
  const vpsRenew: any = useMemo(() => {
    if (vpsList.length) {
      return vpsList?.reduce((curr: any, it: any) => {
  ...
  const handleNavigateToDetail = (id: string) => {
    navigate(`${paths.vps_management_vietstack}/${id}/overview`);
  };
  ...
  const handleRenewVPS = async (item: any, value: any) => {
    ...
    dispatch(asyncThunkPaginationVpsVietStack(search));
    showToast(
...
export default TableVpsVietStack;
