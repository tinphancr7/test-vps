...
function NameCheapList({ domains, error, name, index, isActive }: any) {
  ...
  const handleLoadMore = async () => {
    ...
    try {
      ...
      const { data } = await suppliesDomainApi.loadMoreNameCheap({
      ...
      if (data?.status === 1) {
        ...
        if (data?.domains?.length === MAX_SIZE_DATA) {
          setHasMore(true);
        } else {
          setHasMore(false);
        }
      ...
      <div className="flex flex-col gap-3 w-full">
        {error ? (
          ...
          domainsList?.map((domain: any, index: number) => (
            <div
              key={index}
              className="flex justify-between items-center w-full px-3 border-[1.5px]  border-gray-100 py-4 rounded"
            >
                ...
                <div className="flex flex-col gap-2 justify-end items-end">
                  {!domain?.available || domain?.available === "no" ? (
                    <>
                      ...
                      <p className="text-black text-base tracking-wide">Tên miền đã được đăng kí</p>
                    </>
                  ) : (
                    <>
