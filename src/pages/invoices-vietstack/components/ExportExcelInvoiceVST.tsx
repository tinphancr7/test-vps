...
function ExportExcelInvoiceVST({ dataInvoice }: any) {
  const [isLoading, setIsLoading] = useState(false);
  const fetchData = async () => {
    try {
      const dataFetch = dataInvoice;
      if (dataInvoice.from) {
        dataFetch.from = Number(dataInvoice.from) * exchangeRateVST;
      }
      if (dataInvoice.to) {
        dataFetch.to = Number(dataInvoice.to) * exchangeRateVST;
      }
      if (dataInvoice.time) {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Danh sách hóa đơn CLOUD 1");
      ...
      if (buffer) {
        ...
        a.href = url;
        a.download = `Hóa đơn Cloud 01.xlsx`;
        a.click();
...
export default ExportExcelInvoiceVST;
