...
function ExportOrderExcel() {
  ...
  const fetchData = async () => {
    try {
      ...
      const { data } = await vpsApis.getPagingVpsVietStackOrVng({
        ...query,
        type: VpsTypeEnum.VIETSERVER_VPS,
      });
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Quản lý VPS - VietServer");
      ...
      headerRowTotalVps.eachCell({ includeEmpty: true }, (cell: any) => {
        cell.font = { bold: true };
      ...
      headerRowTotalPrice.eachCell({ includeEmpty: true }, (cell: any) => {
        cell.font = { bold: true };
        cell.alignment = { vertical: "middle", horizontal: "center" };
        cell.fill = {
          type: "pattern",
      ...
      if (buffer) {
        ...
        a.href = url;
        a.download = `quan-ly-vps-vietserver.xlsx`;
        a.click();
