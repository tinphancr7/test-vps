...
function GodaddysList({ domains, error, name, index, isActive }: any) {
  const dispatch = useAppDispatch();
    ...
    const handleLoadMore = async () => {
        ...
        try {
            ...
            const { data } = await suppliesDomainApi.loadMoreGodaddy({
    ...
    const isExistingInCart = (info: any) => domainsInCart?.find(
        (it: any) => it?.domainProvider === info?.supplier && it?.domain === info?.domainName
    );
    ...
    const handleAddToCart = async (info: any) => {
        ...
        const isExisting = !domainsInCart?.length 
            ? false 
            : isExistingInCart(info);
    ...
    return (
        <div className="flex flex-col gap-6 rounded border-gray-100 border-1 p-4 shadow-lg">
            <div className={`py-4 flex items-center gap-2 text-gray-800 font-semibold text-[15px] ${isActive ? 'sticky top-0 bg-white' : ''}`}>
                            ...
                            <div className="flex gap-8 items-center justify-center">
                                ...
                                {domain?.available === 'yes' && (
                                    <Button
                                        color="primary"
                                        variant="solid"
                                        className={`rounded-full w-8 min-w-0 h-8 ${isExistingInCart(domain) ? 'bg-danger' : 'bg-primary-500'}`}
