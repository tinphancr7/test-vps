...
import ExportExcelInvoiceVNG from "./components/ExportExcelInvoiceVNG";
...
export default function InvoicePage() {
  ...
  const handleFetchInvoice = useCallback(
    ({ page, limit, search, provider, topFilter }: any) => {
      const { time, team, from, to } = topFilter || {};
        ...
        fetchInvoice({
          ...
          team:
            user?.role?.name?.toLowerCase() === "leader" && !team
              ? user?.team?.join(",")
              : team,
          from: from ? Number(from) * exchangeRateVNG : from,
          to: to ? Number(to) * exchangeRateVNG : to,
        })
      );
    },
    [dispatch]
  ...
  const renderCell = useCallback((item: any, columnKey: any) => {
    ...
    switch (columnKey) {
      case "invoice_id":
        return (
          <p className="text-bold text-black cursor-pointer">{cellValue}</p>
        );
      case "ip":
      ...
      //         {addCommas(convertVnToUsd(cellValue, "VNG") || "") || 0} $
      //       </p>
      //     );
      case "price": {
        let total = 0;
        item["items"].map((item: any) => {
          const price = convertVnToUsd(item?.amount, "VNG");
          total += price;
        });
          ...
          <div className="relative flex items-center justify-center gap-4">
            ...
            >
              <Button
                ...
                onClick={() =>
                  navigate(paths.invoices_vng_detail, {
    ...
    const fetchProvider = async () => {
      ...
      if (response?.data?.data?.data?.length > 0) {
        const objData = response?.data?.data?.data?.find((item: any) => {
          return item._id === ProviderIDEnum.VNG;
        });
        setProvider(objData);
      }
    };
    ...
    <Access subject={SubjectEnum.INVOICE} action={ActionEnum.READ}>
      <div>
        ...
        <CustomTable
          ...
          isLoading={isLoading}
          title="Danh sách hóa đơn Cloud 02"
        />
      </div>
    </Access>
  );
