...
function ConfirmBuyLoadBalancer() {
  ...
  const handleCreateLoadBalancer = async () => {
    setIsLoading(true);
    const create = await digitalOceanBuCloudApi.createLoadBalancerDigitalOcean(
      objectCreateLoadBalancer
    );
    if (!create?.data?.status) {
      setIsLoading(false);
      if (create?.data?.message === "user_is_not_enough_credit") {
        showToast(
          "Lỗi tạo Load Balancer, Tài khoản không đủ tiền để tạo ",
          "error"
        );
        return;
      }
