...
function ExportOrderExcel() {
  const tableVpsVng = useAppSelector((state) => state.table["vps_general"]);
  const { status, teamSelected, searchByIp, productName, dueDate, provider, site, os, dateRange } =
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Quản lý VPS tổng quan");

      worksheet.addRow([]);

      const headerRowTotalVps = worksheet.addRow(["Tổng số VPS:", dataExport.total]);
      ...
      const headerRowTotalPrice = worksheet.addRow(["Tổng số tiền:", calculatorTotal]);
        ...
        const rowData = columns.map((col) => {
          switch (col._id) {
            ...
            case "total":
              ...
              const calculatorTotal = item["vps_id"]?.["total"] + item["vps_id"]?.["total"] * 0.1;
              return convertVnToUsdFollowProviderId(
                calculatorTotal,
            ...
            case "provider":
              return item["vps_id"]?.["provider"]?.["name"] || "(Trống)";
            case "team":
      ...
      if (buffer) {
        ...
        a.href = url;
        a.download = `quan-ly-vps-tong-quan.xlsx`;
        a.click();
  ...
  return (
    <Button
      ...
      startContent={<RiFileExcel2Line className="text-white min-w-max min-h-max text-lg" />}
