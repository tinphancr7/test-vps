...
function ModalConfirmDestroy({
  ...
  const handleDestroyLoadBalancer = async () => {
    setIsLoading(true);
    const result = await digitalOceanBuCloudApi.destroyLoadBalancerDigitalOcean(
      info
    );
    setIsLoading(false);
      ...
      <ModalContent>
        {(onClose) => (
          <>
            ...
            <ModalBody>
              <p>
                Nhập tên của Load Balancer để hủy vĩnh viễn Load Balancer này
              </p>
              <Input
                type="text"
                label=""
