...
import { getOsDefaultUbuntu, osTemplate } from "@/constants/os-templates";
...
function InitializeVPSBuCloud() {
	const dispatch = useAppDispatch();
	const navigate = useNavigate();
	const location = useLocation();
	const product = location?.state.item; // Lấy giá trị từ `state`
	const keyTab = location?.state.keyTab; // Lấy giá trị từ `state`
	// const { product }: any = useAppSelector((state) => state.prodVng);
	...
	useEffect(() => {
		(async () => {
			...
			document.title = `BuCloud: Khởi tạo VPS - ${product?.name}`;
		})();
	...
	const handleSubmit = async () => {
		try {
			...
			if (data?.status === 1) {
				showToast("Tạo VPS - VNG thành công!", "success");
			...
			switch (error?.response?.data?.status) {
				...
				default:
					showToast("Tạo VPS - BuCloud thất bại!", "error");
					break;
			}
		} finally {
			setIsLoading(false);
	...
	const totalPriceVAT: any = (totalPrice * 1.1).toFixed(2);
	return (
				...
				<Container className="col-span-3 sticky top-0 h-max flex flex-col gap-5">
					<div className="flex items-center justify-between gap-4">
						<Button
							...
							onPress={() => navigate(paths.bu_cloud, { state: keyTab })}
						>
							<HiOutlineArrowLeft />
						</Button>
						...
						<div className="mt-1 before:block before:absolute before:-inset-1 before:-skew-y-6 before:bg-primary relative inline-block">
							<div className="text-2xl font-bold tracking-wide text-right text-white -rotate-6">
								<b className="relative">
									{formatPrice(convertVnToUsd(product?.m, "BuCloud"))}
					...
					<div className="shadow-container py-2 px-4 rounded-md">
						...
						<div className="flex items-center justify-between gap-3 mt-5">
							...
							<div className="text-lg font-semibold tracking-wide text-center text-primary">
								<b className="relative">
									{formatPrice(convertVnToUsd(price, "BuCloud"))}
						...
						<div className="flex items-center justify-between gap-3 mt-5">
							...
							<div className="text-lg font-semibold tracking-wide text-center text-primary">
								<div className="flex gap-5">
									<b className="relative">
										{formatPrice(
											convertVnToUsd(totalPrice * (rateVAT / 100), "BuCloud")
										)}
										<span className="absolute top-0 -right-3 text-sm">$</span>
									</b>
								</div>
					...
					<div className="flex items-center justify-between gap-3 px-4 py-2 shadow-container rounded-md">
						...
						<div className="text-lg font-semibold tracking-wide text-center text-primary mr-3">
							<b className="relative">
								{formatPrice(convertVnToUsd(totalPriceVAT, "BuCloud"))}
								<span className="absolute top-0 -right-3 text-sm">$</span>
							</b>
						</div>
					</div>
					...
					<CreateOrder
						...
						providerId={ProviderIDEnum.BuCloud}
...
export default InitializeVPSBuCloud;
