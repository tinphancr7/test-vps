...
import { asyncThunkGetPagingAlibabaEcs } from "@/stores/async-thunks/alibaba-ecs-thunk";
...
function FilterTableAlibabaEcs() {
  ...
  useEffect(() => {
    ...
    if (tableVpsAlibabaEcs) {
      const cPageSize = tableVpsAlibabaEcs?.pageSize
        ? // eslint-disable-next-line no-unsafe-optional-chaining
          [...tableVpsAlibabaEcs?.pageSize][0]
        : 10;
      ...
      query.pageIndex = Number(tableVpsAlibabaEcs?.pageIndex) || 1;

      dispatch(asyncThunkGetPagingAlibabaEcs(query));
    }

    return () => {};
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tableVpsAlibabaEcs, status, teamSelected, searchMatch]);
...
export default FilterTableAlibabaEcs;
