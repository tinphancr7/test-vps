...
function ActionsVps({ id, item }: { id: string; item: any }) {
    ...
    useEffect(() => {
        setStatusVps(item?.state?.name);

        const arrKeys: any = [];

        if (
            !item?.state ||
            ["pending", "stopping"].includes(item?.state?.name)
        ) {
    ...
    const handleShutdownVps = async () => {
        try {
            ...
            const { data } = await awsLightsailApis.stopInstance(id);
    ...
    const handleRebootVps = async () => {
        try {
            ...
            setStatusVps("pending");
            const { data } = await awsLightsailApis.rebootInstance(id);
    ...
    const handleStartVps = async () => {
        try {
            ...
            setStatusVps("pending");
            const { data } = await awsLightsailApis.startInstance(id);
