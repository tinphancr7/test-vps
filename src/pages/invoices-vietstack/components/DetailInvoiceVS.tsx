...
const DetailInvoiceVS: React.FC = () => {
    ...
    const fetchProvider = async () => {
      ...
      if (response?.data?.data?.data?.length > 0) {
        const objData = response?.data?.data?.data?.find(
          (item: any) => item._id === ProviderIDEnum.VIET_STACK
        );
        setProvider(objData);
      }
    };
    ...
    <div className="h-[90vh] overflow-hidden overflow-y-auto">
      <div className="mx-auto flex flex-col items-center justify-center py-2">
        ...
        <div className="flex items-center justify-between w-full max-w-[800px] my-4">
          <Button
            className="bg-red-500 text-white uppercase text-sm"
            onClick={() => navigate(paths.invoices_vietstack)}
          >
            Quay lại
          </Button>
          <Button
        ...
        <div
          className="border border-gray-200 w-[800px] min-w-[800px] min-h-[900px] bg-no-repeat bg-contain"
          style={{
            backgroundImage: `url("/imgs/background-vs.png")`,
          }}
          ref={contentRef}
        >
          {isLoading ? (
            ...
            <div className="px-12 py-20">
              {/* Header Section */}
              <div className="mb-12 flex items-center justify-between font-bold">
                <Image
                  src="/imgs/logo-vietstack.png"
                  alt="VNG Logo"
                  className="w-[170px] h-[68px]"
                />
                <p> CÔNG TY TNHH CÔNG NGHỆ VIETSTACK</p>
              </div>

              <div className="grid grid-cols-12 gap-10">
                {/* Invoice Details */}
                <div className="col-span-6 text-black uppercase space-y-2 text-sm font-bold">
                  <div className="flex items-center justify-between">
                    <span>HÓA ĐƠN</span>
                    <span>{firstInvoice?.invoice_id || ""}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span>NGÀY LẬP HÓA ĐƠN</span>
                    <span>{firstInvoice?.invoice?.date_created || ""}</span>
                  </div>
                  <div className="flex items-center justify-between">
                ...
                <div className="col-span-6 text-black uppercase space-y-2 text-sm font-bold">
                  ...
                  <div className="flex items-center justify-between">
                    <Button color="success" className="text-white text-xs">
                      ĐÃ THANH TOÁN
                    </Button>
                  </div>
                </div>
              ...
              <div className="mt-20 text-sm">
                ...
                <div className="bg-[#fde7e9] w-full p-4 font-bold flex items-center justify-between">
                ...
                <div>
                  {invoices.map((item, index) => (
                    ...
                    >
                      <div>
                        {`IP (CPU: ${item.invoice?.vm?.cpus || 0} Core - RAM: ${
                          Math.floor(item.invoice?.vm?.memory / 1024) || 0
                        } GB - SSD: ${item.invoice?.vm?.disk || 0} GB)`}
                      </div>
                      <div>{item.invoice?.vm?.ipv4 || ""}</div>
                    </div>
                  ))}
                </div>

                {/* Grouped Items */}
                {Object.entries(groupItems).map(([key, group]) => (
                  <div
                    key={key}
                    className="bg-[#fde7e9] w-full p-4 font-bold flex items-center justify-between"
                  >
                    <div>
                      CLOUD VPS {group[0].invoice?.vm?.cpus || 0}-
                      {Math.floor(group[0].invoice?.vm?.memory / 1024) || 0}
                    </div>
                    <div>
                      {addCommas(convertVnToUsd(group[0]?.invoice?.total)) || 0}{" "}
                      x {group.length} ={" "}
                      {addCommas(
                        convertVnToUsd(group[0]?.invoice?.total * group.length)
                      ) || 0}{" "}
                      $
                    </div>
                  </div>
                ...
                <div className="bg-white flex flex-col items-end space-y-2 font-bold p-4">
                  ...
                  <div className="">
                    ...
                    <span className="w-[150px] text-right inline-block">
                      {addCommas(convertVnToUsd(totalAmount) || "") || 0} $
                    </span>
                  </div>
                  <div className="">
                    <span>THUẾ</span>
                    <span className="w-[150px] text-right inline-block">
                      {addCommas(convertVnToUsd(taxAmount))} $
                    </span>
                  </div>
                  <div className="">
                    <span>GIẢM GIÁ</span>
                ...
                >
                  <div className="flex items-center justify-end font-bold">
                    ...
                    <span className="w-[150px] text-right">
                      {addCommas(
                        convertVnToUsd(Math.round(grandTotal)) || ""
                      ) || 0}{" "}
                      $
                    </span>
                  </div>
                ...
                <div className="flex justify-end mt-4">
                  <div className="flex flex-col  p-4 space-y-1 rounded-lg bg-[#f8f8ff]">
                    <p className="font-bold">Thông tin chuyển khoản</p>
                    <p>
                      <span className="font-bold"> Ngân hàng</span>: ACB
                    </p>
                    <p>
                      <span className="font-bold">Số tài khoản</span>: 12365668
                    </p>{" "}
                    <p>
                      <span className="font-bold">Chủ tài khoản</span>: CÔNG TY
                      TNHH CÔNG NGHỆ VIETSTACK
                    </p>
                  </div>
                </div>
              </div>
...
export default DetailInvoiceVS;
