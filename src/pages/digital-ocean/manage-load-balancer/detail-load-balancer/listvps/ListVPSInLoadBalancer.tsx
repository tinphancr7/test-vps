...
function ListVPSInLoadBalancer({ info, setRender }: any) {
    ...
    const handleRemoveVps = async (item: any) => {
        setLoading(true);
        const result = await digitalOceanApi.removeVPSLoadBalancerDigitalOcean(
            item?.droplet_id,
            info
        );
        setLoading(false);
        if (!result?.status) {
            showToast(
                "Remove VPS ra khỏi Load Balancer không thành công",
                "error"
            );
            return;
        }
                    ...
                    <div className="text-left">
                        <div className=" flex gap-4">
                            ...
                            <p className="font-bold text-[#0069ff]">
                                {item.status !== "new" ? (
                                    <Link
                                        to={`/vps/digital-ocean/${item?._id}/overview`}
                                    >
                                        {item?.nameVPS}
                                    </Link>
                                ) : (
