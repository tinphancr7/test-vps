...
import { asyncThunkGetProductById } from "@/stores/async-thunks/prod-vng-thunk";
...
function InitializeVps() {
	...
	useEffect(() => {
		(async () => {
			...
			if (status === 1) {
				document.title = `VNG: Khởi tạo VPS - ${data?.name}`;
			}
		})();
	...
	const handleSubmit = async () => {
		try {
			...
			switch (error?.response?.data?.status) {
				...
				default:
					showToast("Tạo VPS - VNG thất bại!", "error");
					break;
			}
		} finally {
			setIsLoading(false);
				...
				<Container className="col-span-3 sticky top-0 h-max flex flex-col gap-5">
					...
					<div className="flex items-center justify-between gap-4">
						...
						<Button
							...
							onPress={() => navigate(paths.vps_vng)}
						>
							<HiOutlineArrowLeft />
						</Button>
					...
					<div className="flex items-center justify-between gap-3 px-4 py-2 shadow-container rounded-md">
						...
						<div className="text-lg font-semibold tracking-wide text-center text-primary mr-3">
							<b className="relative">
								{formatPrice(
									convertVnToUsd(totalPriceVAT, "VNG") +
										convertVnToUsd(totalPriceOriginal, "VNG")
								)}
								<span className="absolute top-0 -right-3 text-sm">$</span>
							</b>
						</div>
