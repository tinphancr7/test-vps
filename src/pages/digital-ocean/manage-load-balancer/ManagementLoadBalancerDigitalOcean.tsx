...
import {
  getListLoadBalancer,
  setSearch,
} from "@/stores/slices/digital-ocean-slice/digital-ocean-load-balancer.slice";
...
function ManagementLoadBalancerDigitalOcean() {
  ...
  const { listLoadBalancer, total, isLoading } = useAppSelector(
    (state) => state.digitalOceanLoadBalancer
  );
  ...
  const handleDetailLoadBalancer = (item: any) => {
    if (item?.status !== "new") {
      navigate(`/vps/load-balancer-digital-ocean/${item?._id}/overview`);
    } else {
      showToast(
          ...
          <div className="text-left">
            <div className=" flex gap-4">
              ...
              <p className="font-bold text-[#0069ff]">
                {item.status !== "new" ? (
                  <Link
                    to={`/vps/load-balancer-digital-ocean/${item?._id}/overview`}
                  >
...
export default ManagementLoadBalancerDigitalOcean;
