...
function TableVpsVng() {
  ...
  const columns = [
    ...
    { _id: "total", name: "Giá", sortable: true },
  ...
  const handleNavigateToDetail = useCallback(
    (id: string) => {
      navigate(`${paths.vps_manage_bu_cloud}/${id}/overview`);
    },
    [navigate]
  );
  ...
  const handleRenewVPS = async (item: any, value: any) => {
    ...
    dispatch(asyncThunkPaginationVpsBuCloud(search));
    showToast(
  ...
  const renderCell = (item: any, columnKey: string) => {
    ...
    switch (columnKey) {
            ...
            <div>
              ...
              <div className="flex items-center flex-nowrap gap-1">
                <div className="flex items-center gap-1">
                  <Avatar
                    src={findOs?.img || "/imgs/icon_win.svg"}
      ...
      case "total":
        ...
        const calculatorTotal = cellValue + cellValue * 0.1;

        return (
          <p className="font-medium tracking-wider">
            {formatPrice(convertVnToUsd(calculatorTotal, "BuCloud"))}
            <b>$</b>
          </p>
        );
