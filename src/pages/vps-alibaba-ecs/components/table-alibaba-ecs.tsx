...
function TableAlibabaEcs() {
  ...
  const handleNavigateToDetail = (id: string) => {
    navigate(`${paths.vps_management_alibaba_ecs}/${id}/overview`);
  };
  ...
  const renderCell = (item: any, columnKey: string) => {
    ...
    const findImage = Object.keys(publicPlatform)?.find((it) =>
      it.toLowerCase().includes(imageName.toLowerCase())
    ) as string;
    ...
    switch (columnKey) {
        ...
        return (
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-1">
              <img
                src={`/imgs/alibaba-ecs/${
                  (publicPlatform as any)[findImage]?.icon
                }`}
                className="w-[24px]"
              ...
              <div className="text-base text-left">
                <b>{item["vps_id"]?.Cpu}</b> vCPUs -{" "}
                <b>{(item["vps_id"]?.Memory / 1024).toFixed(1)}</b> GB RAM -{" "}
                <b>{item["vps_id"]?.Size || 0}</b> GB DISK
              </div>
            </div>
          </div>
        );
      ...
      case "price":
        ...
        const calculatorTotal = item["vps_id"]?.Price * 1.1;
      ...
      case "actions":
        return (
          <div className="flex justify-center gap-2">
            ...
            {!isTP && <ActionsVps item={item["vps_id"]} />}
          </div>
        );
...
export default TableAlibabaEcs;
