...
function FormDns({
  ...
  const [dnsType, setDnsType] = useState<any>(new Set([TYPE_DNS_OPTIONS[0]]));
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [comment, setComment] = useState("");

    const fetchData = async () => {
        try {
            const { data } = await cloudflareApis.getDetailDnsById(item?.id, zone_id);

            if (data?.status === 1) {
                const newState = (FORM_DNS as any)[data?.result?.type];

                setDnsType(new Set([data?.result?.type]));
                setState(newState);

                for (const key of Object.keys(newState)) {
                    let parseValue = data?.result[key];
  ...
  const handleValueChange = (key: string, value: any) => {
    if (key === "type_dns") {
      ...
      if (isEdit && item?.type === parseValue) {
        fetchData();
      }
    } else {
      setValue(key, value);
    ...
    <div className="h-full relative flex flex-col justify-between overflow-y-auto scroll-main overflow-x-hidden">
      <div className="grid grid-cols-2 gap-3 pb-4 pr-2">
        <Select
          ...
          label={"Type"}
          items={
            TYPE_DNS_OPTIONS?.map((it) => ({ label: it, value: it })) || []
          }
        ...
        {Object.keys(initialState).map((key, index) => {
          ...
          if (getState(key).type === "select")
            return (
              <Select
                ...
                classNames={{
                  base: `w-full ${
                    ["lat_direction", "long_direction"].includes(key)
                      ? "col-span-1"
                      : "col-span-2"
                  }`,
                  label: "text-dark font-medium",
              ...
              >
                {(getState(key)?.options as Array<any>).map((item) => (
                  <SelectItem key={item.value} textValue={item.value}>
                    {item?.label}
                  </SelectItem>
                ))}
          ...
          return (
            <div className="col-span-1 py-2">
              <span className="font-medium mb-1 text-base">
                {getState(key).label}:{" "}
              </span>
