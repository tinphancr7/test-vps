...
function ModalAssignedUserToVps({ isOpen, onOpenChange }: any) {
  ...
  const tableVps = useAppSelector((state) => state.table["vps_up_cloud"]);
  const [listVps, setListVps] = useState<any[]>([]);
  const [listUser, setListUser] = useState<any[]>([]);
  const getInfoListVPS = async () => {
    try {
      setStateLoading((prev) => ({ ...prev, isLoadingVps: true }));
      if (!tableVps?.selectedKeys) return;
      const selectedKeys = tableVps?.selectedKeys;
      const listKey = [...selectedKeys];

      const result = await vpsApis.getListVpsAdmin(listKey.join(","));
  ...
  const handleSubmit = async () => {
    try {
      ...
      await vpsApis.updateAssignedVpsToUser(listVpsid, listUserId);
                      ...
                      <div className="flex w-full gap-2 flex-wrap">
                        {listVps.slice(0, 5).map((vps, index) => (
                          <Chip
                            ...
                            color="primary"
                          >
                            {vps?.vps_id?.ip_addresses?.ip_address?.find(
                              (ip: any) =>
                                ip.family === "IPv4" && ip.access === "public"
                            )?.address ||
                              vps?.vps_id?.domain ||
                              "(Trống)"}
                          </Chip>
