...
export const getListLoadBalancer: any = createAsyncThunk<Pagination, any>(
  ...
  async (query, { rejectWithValue }) => {
    try {
      const response =
        await digitalOceanBuCloudApi.managementLoadBalancerDigitalOcean(query);
...
const digitalOceanLoadBalancerBuCloudSlice = createSlice({
  name: "digitalocean-bucloud",
  initialState,
  reducers: {
...
export const { setListLoadBalancer, setSearch } =
  digitalOceanLoadBalancerBuCloudSlice.actions;

export default digitalOceanLoadBalancerBuCloudSlice.reducer;
